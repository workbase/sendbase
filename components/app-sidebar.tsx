...
import { FileTextIcon, LogOutIcon, PenLineIcon, SettingsIcon, SparklesIcon } from "lucide-react"
    ...
    <Sidebar variant="sidebar" collapsible="offcanvas" className="border-r border-sidebar-border">
      <SidebarHeader className="gap-3 border-b border-sidebar-border p-4">
        <Link href="/dashboard" className="flex items-center gap-2.5">
          ...
          <span>
            <span className="block text-sm font-semibold tracking-tight">Sendbase</span>
            <span className="block text-xs text-muted-foreground">한 번 쓰고, 모든 곳에</span>
          </span>
        </Link>
        <Button render={<Link href="/dashboard" />} className="w-full justify-center" size="lg">
      ...
      <SidebarContent>
        <SidebarGroup className="px-2 py-3">
          <SidebarGroupLabel className="px-2 text-xs">최근 게시물</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {posts.length === 0 ? (
                <li className="px-2 py-8 text-center text-xs leading-5 text-muted-foreground">
                  아직 게시물이 없습니다.<br />첫 게시물을 작성해 보세요.
                </li>
              ) : (
                posts.map((post) => (
      ...
      <SidebarFooter className="border-t border-sidebar-border p-3">
        ...
        <div className="mt-2 flex items-center gap-2 rounded-lg bg-sidebar-accent/60 p-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <Badge variant="outline" className="mt-0.5 h-4 px-1.5 text-[0.65rem]">Creator</Badge>
          </div>
