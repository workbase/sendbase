    ...
    <article>
      ...
      <DocumentSection title="2. 계정과 이용자의 책임">
        <p>
          ...
          수 있습니다. 로그인 계정의 관리와 인증 수단의 보안은 사용자의
          책임입니다.
        </p>
        <p>
      ...
      <DocumentSection title="4. 게시와 외부 서비스">
        <p>
          사용자가 게시를 실행하면 선택한 채널로 제목, 본문 및 첨부 이미지를
          전송합니다. 게시 결과와 노출 상태는 각 외부 플랫폼의 시스템, 정책 및
          네트워크 상태에 따라 달라질 수 있습니다.
        </p>
        <p>
          사용자는 연결된 플랫폼의 이용약관과 정책을 함께 준수해야 합니다. 외부
          플랫폼의 제한, 삭제, 장애 또는 정책 변경으로 발생한 게시 실패나 손해에
          대해 센드베이스는 법령이 허용하는 범위에서 책임을 제한합니다.
        </p>
      </DocumentSection>
      <DocumentSection title="5. 이용 제한과 종료">
        <p>
          서비스의 안정성 또는 법령 준수를 위해 부정 이용, 보안 위협, 권리
          침해가 확인되거나 합리적으로 의심되는 경우 이용을 제한할 수 있습니다.
          사용자는 설정에서 플랫폼 연결을 해제할 수 있으며, 계정 삭제 기능으로
          계정과 서비스 내 데이터를 삭제할 수 있습니다.
        </p>
      </DocumentSection>
    </article>
  )
  ...
  return (
    <article>
      ...
      <DocumentSection title="2. 처리 목적과 방식">
        <p>
          수집한 정보는 로그인과 세션 유지, 연결한 플랫폼 계정 식별, 사용자가
          요청한 콘텐츠 게시, 게시 이력 표시, 연결 설정 관리 및 보안 검증에만
          사용합니다. OAuth 인증 요청의 state 값은 위조 요청 방지를 위해 짧은
          기간의 HTTP 전용 쿠키로 보관하며, X 연결에는 PKCE 검증값을 함께
          사용합니다.
        </p>
        <p>
          액세스 토큰과 갱신 토큰은 서버에서 암호화하여 저장하며 브라우저에
          제공하지 않습니다. 토큰은 게시 또는 필요한 갱신 시에만 외부 플랫폼
          API에 전송합니다.
        </p>
      </DocumentSection>
      ...
      <DocumentSection title="4. 제3자 제공 및 처리 위탁">
        <p>
          사용자가 선택한 게시 대상에 한해 콘텐츠와 게시에 필요한 인증 정보를
          Threads, X, Discord 등 해당 외부 플랫폼으로 전송합니다. 로그인 시에는
          선택한 OAuth 제공자와 인증 정보를 교환합니다. 서비스 데이터와 이미지
          저장을 위해 Supabase의 데이터베이스 및 저장소를 이용합니다.
        </p>
      </DocumentSection>
      <DocumentSection title="5. 이용자의 권리와 문의">
        <p>
          사용자는 설정에서 플랫폼 연결을 해제하고, 계정 삭제를 통해 서비스 내
          개인정보의 삭제를 요청할 수 있습니다. 외부 플랫폼의 OAuth 권한 철회는
          해당 플랫폼의 계정 설정에서 직접 진행해야 합니다.
        </p>
    ...
    <LegalTable>
      ...
      <TableBody>
        <TableRow>
          ...
          <TableCell className="whitespace-normal">
            계정 식별자, 표시 이름, 프로필 이미지, OAuth 토큰과 만료 정보, 세션
            토큰
          </TableCell>
        ...
        <TableRow>
          ...
          <TableCell className="whitespace-normal">
            연결 플랫폼, 외부 계정 식별자·표시 이름, 게시 토큰·만료 정보,
            Discord 웹훅 정보, 네이버 카페·SOOP 게시판 설정
          </TableCell>
          <TableCell className="whitespace-normal">
            계정 연결 및 게시 대상 관리
          </TableCell>
      ...
      <LegalTable>
        ...
        <TableBody>
          <TableRow>
            ...
            <TableCell className="whitespace-normal">
              계정 유지 기간 동안 보관하며, 계정 삭제 시 데이터베이스에서 함께
              삭제합니다.
            </TableCell>
          </TableRow>
          <TableRow>
            ...
            <TableCell className="whitespace-normal">
              공개 이미지 저장소에 저장되어 게시와 미리보기를 위한 공개 URL로
              제공될 수 있으며, 계정 삭제 시 삭제를 시도합니다.
            </TableCell>
          </TableRow>
          <TableRow>
            <TableCell>외부 플랫폼 게시물</TableCell>
            <TableCell className="whitespace-normal">
              서비스에서 계정을 삭제해도 외부 플랫폼의 게시물은 남으므로 해당
              플랫폼에서 별도로 삭제해야 합니다.
            </TableCell>
          </TableRow>
        </TableBody>
