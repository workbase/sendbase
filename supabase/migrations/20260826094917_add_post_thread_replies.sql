alter table public.posts
  add column thread_replies jsonb not null default '[]'::jsonb;

alter table public.posts
  add constraint posts_thread_replies_is_array
  check (jsonb_typeof(thread_replies) = 'array');
...
begin
  ...
  if 'x'::public.publish_platform = any(p_destinations) then
