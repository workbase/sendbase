alter table public.posts
  add column image_metadata jsonb not null default '[]'::jsonb;

alter table public.posts
  add constraint posts_image_metadata_is_array
  check (jsonb_typeof(image_metadata) = 'array');

create index login_accounts_user_id_idx
  on public.login_accounts (user_id);
